...
mongodb_collection = mongodb["aspectj"]
logger = logging.getLogger()


class AspectJ(scrapy.Spider):
    name = "AspectJ_EclipseBugzilla"
    start_urls = [
        'https://bugs.eclipse.org/bugs/rest/bug?product=AspectJ&status=RESOLVED&status=VERIFIED&status=CLOSED',
        'https://bugs.eclipse.org/bugs/rest/bug?product=AspectJ&status=RESOLVED&status=VERIFIED&status=CLOSED&limit=2000&offset=2000',
    ]
